...
- Before modifying code, establish the root cause, identify the complete affected behavior and its state transitions, and define how the original report will be verified through its real entrypoint. Do not stack speculative or symptom-level patches. If a change exposes another failure or invalidates an earlier assumption, stop, reassess the whole flow, and update the diagnosis before making another change.
- Treat every user instruction literally. Do not perform any operation that violates the instructions or scope established by the applicable `AGENTS.md` files. Do not infer related workflow actions, repository operations, refactors, cleanup, or follow-up work. Before acting, check the intended operation against those rules; if it conflicts with them or would materially change the established state or scope, do not perform it.
- Verification must cover the complete reported flow after the final change, including repeated execution and persisted or reloaded state when applicable. Passing isolated tests or fixing the latest visible symptom is not sufficient evidence of completion.

## Core Expectations
- Do not commit or push unless the user explicitly asks.
- When the user explicitly asks the agent to create a commit, first create the requested commit, then derive its shortest unique Git abbreviation with `git rev-parse --short=4 HEAD`. Git uses at least four hexadecimal characters and lengthens the abbreviation when necessary for uniqueness. Create `_docs/commits/{identifier}.md` with a brief summary, stage that file, and amend the commit so its subject starts with the exact same identifier followed by a space, for example `243b Update: describe the change`. Preserve the rest of the original message. The identifier remains the initial commit's abbreviation even though amending changes the internal Git SHA. Do not overwrite an existing summary file; increase the abbreviation length until the filename is unused. This generated commit summary is an expected repository artifact and is exempt from the general restriction on unsolicited documentation files.
